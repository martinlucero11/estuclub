...
export default function BenefitCard({ benefit, className, variant = 'grid', priority = false, activeTab }: BenefitCardProps) {
  ...
  const isLocked = useMemo(() => {
    ...
    if (benefit.isCincoDosOnly && !isCincoDos) return true;
    if (benefit.isStudentOnly && roles.length > 0 && !roles.includes('user_student') && !roles.includes('admin')) return true;
    return false;
  }, [benefit.minLevel, userLevel, benefit.isCincoDosOnly, benefit.isStudentOnly, isCincoDos, isAdmin, roles]);
  ...
  const cardContent = (
    <div className={cn(
      ...
      className
        ...
        <Image
            src={optimizeImage(benefit.imageUrl || DEFAULT_BENEFIT_IMAGE, 800)}
            alt={benefit.title}
            fill
            className={cn(
