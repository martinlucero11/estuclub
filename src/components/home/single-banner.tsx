...
export function SingleBanner({ bannerId }: HomeSection) {
    ...
    const bannerContent = (
         <Card className="relative aspect-[16/9] md:aspect-[21/9] w-full overflow-hidden rounded-2xl group">
            <Image
                src={banner.imageUrl}
                alt={banner.title || 'Banner de EstuClub'}
                fill
            ...
            <div className="absolute inset-0 flex flex-col justify-center items-start p-6 md:p-12 text-white">
                {banner.title && <h3 className="text-2xl md:text-4xl font-extrabold drop-shadow-lg max-w-lg">{banner.title}</h3>}
                {banner.description && <p className="mt-2 text-md md:text-lg drop-shadow-md max-w-md">{banner.description}</p>}
                {banner.link && (
