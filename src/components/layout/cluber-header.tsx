    ...
    <header className="sticky top-0 z-30 w-full bg-white/80 backdrop-blur-xl border-b border-zinc-100 px-4 md:px-8 h-16 md:h-20 flex items-center justify-between">
      ...
      <div className="flex items-center gap-4">
        <Button 
          variant="ghost" 
          size="icon" 
          className="lg:hidden h-10 w-10 rounded-xl hover:bg-zinc-100"
          onClick={onMenuClick}
        >
        ...
        <Logo variant="rosa" className="h-6 w-auto lg:hidden" />
      </div>
      ...
      <div className="flex items-center gap-2 md:gap-4">
        ...
        {dashboardRoles.length > 1 && (
            <div className="hidden md:block">
                <Select onValueChange={(v) => setActiveRole(v as UserRole)} value={activeRole}>
                    <SelectTrigger className="w-[130px] h-10 rounded-xl bg-black/[0.03] border-none font-black text-[9px] uppercase tracking-widest shadow-none">
                        <SelectValue placeholder="Vista" />
                    </SelectTrigger>
                    <SelectContent className="rounded-2xl border-zinc-100 shadow-2xl">
                        {dashboardRoles.map(role => (
                            <SelectItem key={role} value={role} className="font-black text-[9px] uppercase tracking-widest py-3 rounded-xl focus:bg-primary focus:text-white">
                                {role === 'supplier' ? '🛒 Cluber' : `🛡️ ${capitalize(role)}`}
                            </SelectItem>
                        ))}
                    </SelectContent>
                </Select>
        ...
        <div className={cn(
            ...
            <div className="hidden sm:flex flex-col items-end">
                ...
                <span className="text-[7px] font-bold text-zinc-400 uppercase tracking-tighter">Delivery</span>
            </div>
            <Switch 
                checked={!isPaused} 
        ...
        <DropdownMenu>
            <DropdownMenuTrigger asChild>
                <Button variant="ghost" className="h-10 md:h-12 rounded-2xl border border-zinc-100 hover:bg-zinc-50 flex items-center gap-2 px-2 md:px-3">
                    ...
                    <ChevronDown className="h-3 w-3 text-zinc-400 hidden sm:block font-black" />
                </Button>
            </DropdownMenuTrigger>
