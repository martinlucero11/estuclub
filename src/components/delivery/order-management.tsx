    ...
    if (!orders || orders.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center py-20 text-center opacity-50">
                <Package className="h-16 w-16 mb-4 text-foreground" />
                <h3 className="text-xl font-bold">No hay pedidos registrados</h3>
                <p className="text-sm text-foreground">Los pedidos de tus clientes aparecerán aquí.</p>
            </div>
        );
    }
...
const OrderCard = React.memo(({ order, onUpdateStatus }: { 
    ...
    return (
        <Card className="overflow-hidden border-foreground/50 shadow-lg hover:border-primary/20 transition-all duration-300">
            <CardHeader className="pb-3 border-b border-foreground/50 bg-background/30">
                <div className="flex justify-between items-start">
                    <div className="space-y-1">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-[10px] text-foreground font-mono">#{order.id.slice(-6).toUpperCase()}</span>
                        </div>
            ...
            <CardContent className="pt-4 space-y-4">
                ...
                <div className="space-y-3 pt-2">
                    <div className="flex items-start gap-3 p-3 rounded-xl bg-background/50 text-sm">
                        ...
                        <div>
                            ...
                            <p className="text-foreground text-xs">{order.deliveryAddress || 'Retiro presencial'}</p>
                        </div>
                    </div>
                    
                    {order.deliveryNote && (
                        <div className="flex items-start gap-3 px-3 text-xs text-foreground italic">
            ...
            <CardFooter className="bg-background/30 border-t border-foreground/50 gap-2">
                <Button 
                    variant="outline" 
