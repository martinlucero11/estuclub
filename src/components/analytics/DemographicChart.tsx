        ...
        return (
            <div className="bg-white/80 dark:bg-black/80 backdrop-blur-xl border border-black/10 dark:border-white/10 p-4 rounded-2xl shadow-2xl ring-1 ring-black/5">
                ...
                <p className="text-xl font-black text-primary flex items-baseline gap-1">
                    {payload[0].value} <span className="text-[10px] uppercase text-foreground dark:text-white/60 tracking-widest">Usuarios</span>
                </p>
            </div>
        );
    }
    ...
    if (!data || data.length === 0) {
        return (
            <Card className="border-black/10 dark:border-white/10 bg-white/40 dark:bg-white/5 backdrop-blur-xl rounded-[2rem] overflow-hidden h-full shadow-xl">
                ...
                <CardContent className="h-[300px] flex items-center justify-center text-foreground text-xs font-black uppercase tracking-widest">
                    ...
                    {type === 'pie' ? (
                        <PieChart>
                            ...
                            <Legend 
                                ...
                                height={36} 
                                iconType="circle"
                                formatter={(value) => <span className="text-[10px] font-bold text-foreground dark:text-white/70 uppercase">{value}</span>}
                            />
                        </PieChart>
                    ) : (
