...
import { HomeSection, Banner, Benefit, SupplierProfile, Category, benefitCategories, cluberCategories } from '@/types/data';
...
export function HomeSectionForm({ section, onSuccess }: HomeSectionFormProps) {
    ...
    const { data: banners } = useCollectionOnce(query(collection(firestore, 'banners').withConverter(createConverter<Banner>())));
    const { data: benefits } = useCollectionOnce(query(collection(firestore, 'benefits').withConverter(createConverter<Benefit>())));
    const { data: suppliers } = useCollectionOnce(query(collection(firestore, 'roles_supplier').withConverter(createConverter<SupplierProfile>())));
    const { data: announcements } = useCollectionOnce(query(collection(firestore, 'announcements').withConverter(createConverter<Announcement>())));
